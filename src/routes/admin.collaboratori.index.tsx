...
export const Route = createFileRoute("/admin/collaboratori/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Rubrica dei collaboratori con ricerca per nome e hashtag di competenza." },
...
function AdminCollaboratori() {
  ...
  async function createUser(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setNome(""); setCognome(""); setEmail(""); setPassword(""); setShowCreate(false);
      await loadUsers();
    } catch (error) {
  ...
  const realList = users.filter((c) =>
    !query || `${c.nome} ${c.cognome}`.toLowerCase().includes(query) || c.username.toLowerCase().includes(query) || (c.email || "").toLowerCase().includes(query),
  );
  ...
  return (
    <AppShell area="admin" title="Collaboratori">
      <section className="px-3 pt-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Cerca per nome, username o email.</p>
          </div>
          <button type="button" onClick={() => { setShowCreate((value) => !value); setMessage(""); }} className="shrink-0 rounded-lg border border-accent bg-accent px-3 py-2 text-xs font-semibold uppercase tracking-[0.08em] text-accent-foreground active:scale-[0.98]">
        ...
        <section className="mt-5 px-3">
          <form onSubmit={createUser} className="border border-border-strong bg-surface p-5 shadow-[var(--shadow-card)]">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <label className="text-sm text-foreground sm:col-span-2">Email <span className="text-muted-foreground">(opzionale)</span><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 min-h-11 w-full border border-border-strong bg-background px-3" /></label>
            ...
            <button disabled={saving} type="submit" className="mt-4 min-h-11 w-full bg-primary px-4 text-sm font-semibold uppercase tracking-[0.08em] text-white disabled:opacity-60">{saving ? "Creazione…" : "Crea collaboratore"}</button>
          </form>
        </section>
      )}
      ...
      <ul className="mt-5 border-t border-border">
        {loading ? <li className="px-3 py-6 text-sm text-muted-foreground">Caricamento collaboratori…</li> : realList.length > 0 ? realList.map((c) => (
          <li key={c.id}><Link to="/admin/collaboratori/$id" params={{ id: String(c.id) }} className="flex gap-3 border-b border-border px-3 py-3.5 active:bg-muted">
            <Avatar name={`${c.nome} ${c.cognome}`} />
            <span className="min-w-0 flex-1"><span className="block truncate font-serif text-base text-foreground">{c.nome} {c.cognome}</span><span className="block truncate text-xs text-muted-foreground">@{c.username}{c.email ? ` · ${c.email}` : ""}</span></span>
            <span className="eyebrow shrink-0 text-accent">ATTIVO</span>
          </Link></li>
        )) : <li className="px-3 py-6 text-sm text-muted-foreground">Nessun account collaboratore creato.</li>}
      </ul>
    </AppShell>
  );
}
