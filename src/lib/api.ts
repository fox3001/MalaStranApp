...
export async function login(username: string, password: string) {
  const data = await request<{ success: true; token: string; user: AuthUser }>("/api/login", {
    method: "POST",
...
export async function validateSession() {
  ...
  try {
    const data = await request<{ success: true; user: AuthUser }>("/api/me");
    window.localStorage.setItem(USER_KEY, JSON.stringify(data.user));
    return data.user;
  } catch {
    clearAuth();
...
export async function logout() {
  try {
    if (getAuthToken()) await request("/api/logout", { method: "POST", body: "{}" });
  } finally {
    clearAuth();
  }
}
...
export async function createCollaborator(input: { nome: string; cognome: string; email?: string; password?: string }) {
  return request<{ success: true; user: { id: number; nome: string; cognome: string; username: string; email: string | null; role: "user" } }>("/api/admin/users", {
    method: "POST",
...
export async function changeCollaboratorPassword(id: string, password: string) {
  return request<{ success: true }>(`/api/admin/users/${encodeURIComponent(id)}/password`, {
    method: "PATCH",
