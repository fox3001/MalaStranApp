...
export const PROFILE_SKILLS = [
  "Attore", "Rievocatore", "Animatore", "Cavaliere", "Cosplayer",
  "Giocoliere", "Public speaking", "Gestione pubblico", "Gestione bimbimerda",
] as const;

export const PROFILE_COSTUMES = [
  "Potter base", "Medievale base", "Costume completo", "Personale 1", "Personale 2",
] as const;

export type CollaboratorProfile = {
  id: number; nome: string; cognome: string; username: string; email: string | null;
  ruolo: string; created_at: string; phone: string | null; bio: string;
  skills: string[]; costumeFlags: string[];
};

export function buildUsername(nome: string, cognome: string) {
  const normalize = (v: string) => v.trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-zA-Z0-9]+/g, "").toLowerCase();
